...
export default class PlayerSelector extends Component {
  ...
  render() {
      ...
      return (
        <Panel style={{ flex: 0.5 }}>
          <Text style={{ color: "white", textAlign: "center", fontSize: 38 }}>
            {this.props.selectedPlayer.nick}
          </Text>
          <Image
            style={{
              flex: 1,
            ...
            source={
              this.props.selectedPlayer.picture
                ? {
    ...
    return (
      <Panel style={{ flex: 0.5 }}>
        <Text style={{ color: "white", textAlign: "center", fontSize: 38 }}>
          Combatant #{this.props.isPlayerOne ? "1" : "2"}
        </Text>
        <Picker
          selectedValue={
            (this.props.selectedPlayer && this.props.selectedPlayer.id) || null
          }
        ...
        >
          <Picker.Item
            label={`Select player ${this.props.isPlayerOne ? "one" : "two"}`}
            value={""}
          />
          {this.getPickerItems()}
        </Picker>
